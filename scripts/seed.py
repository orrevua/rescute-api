...
def _cats(protector_id) -> list[CatModel]:
    return [
        ...
        CatModel(
            protector_id=protector_id, name="Rosie", age_months=132, sex="female",
        ...
        CatModel(
            protector_id=protector_id, name="Patch", age_months=48, sex="male",
        ...
        CatModel(
            protector_id=protector_id, name="Shadow", age_months=36, sex="male",
        ...
        CatModel(
            protector_id=protector_id, name="Tangerine", age_months=10, sex="female",
...
def _partners() -> list[PartnerModel]:
    return [
        PartnerModel(
            name="Free Paws Veterinary Clinic", address="Rua das Flores, 123", cep="01000-000",
            city="São Paulo", state="SP", description="Partner veterinary clinic",
            discount_pct=10, coupon_code="RESCUTE10",
        ),
        PartnerModel(
            name="Whiskers Pet Shop", address="Av. Paulista, 900", cep="01310-100",
        ...
        PartnerModel(
            name="Meow Cat Hotel", address="Rua do Catete, 45", cep="22220-000",
...
def _donation_posts(protector_id) -> list[DonationPostModel]:
    return [
        ...
        DonationPostModel(
            protector_id=protector_id, title="Winter food supply",
        ...
        DonationPostModel(
            protector_id=protector_id, title="Patch's yearly bloodwork",
...
async def seed() -> None:
    ...
    async with async_session() as session:
        ...
        if not profile:
            ...
            profile.description = "Demo account managing the seed cats."
        ...
        # Rename records earlier seeds created with Portuguese names, so the
        # insert-if-missing checks below match instead of duplicating them.
        for old, new in [
            ("Dona Rosa", "Rosie"), ("Pirata", "Patch"),
            ("Sombra", "Shadow"), ("Tangerina", "Tangerine"),
        ]:
            await session.execute(
                update(CatModel).where(CatModel.name == old).values(name=new)
            )
        for old, new in [
            ("Clínica Patas Livres", "Free Paws Veterinary Clinic"),
            ("PetShop Bigode", "Whiskers Pet Shop"),
            ("Hotelzinho Miau", "Meow Cat Hotel"),
        ]:
            await session.execute(
                update(PartnerModel).where(PartnerModel.name == old).values(name=new)
            )
        for old, new in [
            ("Ração para o inverno", "Winter food supply"),
            ("Pirata's yearly bloodwork", "Patch's yearly bloodwork"),
        ]:
            await session.execute(
                update(DonationPostModel)
                .where(DonationPostModel.title == old)
                .values(title=new)
            )
